...
- **PDF 生成**采用 Markdown → HTML → PDF 管道，完整支持标题、列表、代码块、表格、引用等格式，内嵌 CSS 样式
- 支持中文字体（微软雅黑），排版美观
...
- [x] 多模型供应商配置管理（OpenAI/Anthropic/Google/DashScope/智谱）
- [x] 本地文件搜索（原生索引引擎，AI 自主调用）
- [x] 流式 AI 对话 + 工具调用
- [x] 联网搜索（秘塔 AI）+ 网页阅读
- [x] 文件读写 + 局部编辑 + 目录浏览 + 数据分析
- [x] 对话历史持久化 + AI 记忆系统
- [x] Auto-Coder 自主编码代理 + 命令执行
- [x] 文档生成（PDF Markdown 渲染 + Office 文档 + 回退机制）
- [x] 文件管理 + 应用启动（UAC 智能处理）+ 桌面控制
- [x] MCP 扩展服务（天气/火车票/机票/地图/OCR 等）
- [x] AgentTeam 架构（主 Agent + SubAgent 委派隔离执行）
- [x] Skills 技能系统（用户自定义可复用工具）
- [x] Python 环境自动打包
- [ ] 插件系统
- [ ] 跨平台支持（macOS / Linux）
